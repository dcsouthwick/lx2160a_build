...
case "\$1" in
	start)
		...
		chroot /mnt apt install --no-install-recommends -y systemd-sysv apt dialog nano fdisk locales less wget procps openssh-server ifupdown net-tools isc-dhcp-client isc-dhcp-server ntpdate lm-sensors i2c-tools psmisc less sudo htop iproute2 iputils-ping kmod network-manager iptables rng-tools apt-utils ethtool
...
if [[ ! -f $ROOTDIR/build/debian-bullseye.ext4 ]] && [ "$DISTRO" == "debian" ]; then
